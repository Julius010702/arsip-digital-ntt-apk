...
export default function DetailArsipScreen() {
  ...
  async function handleDelete() {
    Alert.alert('Hapus Arsip', 'Yakin ingin menghapus arsip ini?', [
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
        ...
        <View style={s.card}>
          ...
          {fileUrl ? (
            ...
            <Text style={{ color: COLORS.muted, fontSize: 13 }}>File tidak tersedia</Text>
          )}
        </View>
      </ScrollView>

      <View style={[s.bottomBar, { paddingBottom: insets.bottom + SPACING.md }]}>
        <TouchableOpacity style={[s.btnUnduh, !fileUrl && { opacity: 0.5 }]}
          onPress={() => fileUrl && Linking.openURL(fileUrl)} disabled={!fileUrl}>
...
const s = StyleSheet.create({
  ...
  card:         { backgroundColor: COLORS.white, borderRadius: RADIUS.lg, padding: SPACING.lg, margin: SPACING.lg, marginBottom: SPACING.md, borderWidth: 1, borderColor: COLORS.border, ...SHADOW.sm },
  cardTitle:    { fontSize: 14, fontWeight: '800', color: COLORS.text, marginBottom: 16 },
  fileRow:      { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF5F5', borderRadius: 12, padding: 12, borderWidth: 1, borderColor: '#FEE2E2', gap: 10 },
  ...
  btnHapus:     { width: 52, backgroundColor: COLORS.dangerSoft, borderRadius: 14, justifyContent: 'center', alignItems: 'center' },
